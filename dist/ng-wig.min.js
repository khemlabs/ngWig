...
function pasteHtmlAtCaret(a) {
	...
	if (window.getSelection) {
		if (((b = window.getSelection()), b.getRangeAt && b.rangeCount)) {
			...
			d.innerHTML = a;
			for (var e = document.createDocumentFragment(), f = void 0, g = void 0; (f = d.firstChild); )
				g = e.appendChild(f);
			c.insertNode(e),
				g && ((c = c.cloneRange()), c.setStartAfter(g), c.collapse(!0), b.removeAllRanges(), b.addRange(c));
		}
	} else document.selection && 'Control' != document.selection.type && document.selection.createRange().pasteHTML(a);
}
var VERSION = '3.0.16';
angular.module('ngWig', ['ngwig-app-templates']),
	(angular.ngWig = { version: VERSION }),
	angular.module('ngWig').component('ngWig', {
		...
		controller: [
			...
			function(a, b, c, d, e, f, g, h, i, j) {
				var k = this,
					l = angular.element(b[0].querySelector('#ng-wig-editable'));
				(this.spanish = this.language && 'spanish' == this.language),
					(this.required = 'required' in d),
					(this.isSourceModeAllowed = 'sourceModeAllowed' in d),
					(this.editMode = !1),
					d.$observe('disabled', function(a) {
						(k.disabled = a), l.attr('contenteditable', !a);
					}),
					(this.isEditorActive = function() {
						return l[0] === f[0].activeElement;
					}),
					(this.toggleEditMode = function() {
						(k.editMode = !k.editMode), e.getSelection().removeAllRanges && e.getSelection().removeAllRanges();
					}),
					(this.execCommand = function(a, b) {
						if (k.editMode) return !1;
						if (f[0].queryCommandSupported && !f[0].queryCommandSupported(a))
							throw 'The command "' + a + '" is not supported';
						return 'insertImage' == a
							? void i.openDialogConfirm(
									null,
									g,
									k.spanish ? 'Confirmar' : 'Confirm',
									k.spanish
									...
									function(c) {
										return c
											? void i.openDialog(null, g, { path: 'files/image', type: 'image' }, function(c) {
													l.length && l[0].focus(),
														k.beforeExecCommand({ command: a, options: b }),
														(b = h.apiDomain + 'files/image/' + c),
														f[0].execCommand('insertHtml', !1, '<img src="' + b + '"/>'),
														k.afterExecCommand({ command: a, options: b });
												})
											: k.fileCommand(a, b);
									}
								)
							: 'createlink' === a || 'insertImage' == a ? k.fileCommand(a, b) : void k._execCommand(a, b);
					}),
					(this.fileCommand = function(a, b) {
						(b = e.prompt(k.spanish ? 'Por favor ingrese la URL' : 'Please enter the URL', 'http://')),
							b && k._execCommand(a, b);
					}),
					(this._execCommand = function(a, b) {
						k.beforeExecCommand({ command: a, options: b });
						var c = f[0].getSelection().toString();
						'createlink' === a && '' === c
							? f[0].execCommand('insertHtml', !1, '<a href="' + b + '">' + b + '</a>')
							: f[0].execCommand(a, !1, b),
							k.afterExecCommand({ command: a, options: b }),
							l.length && l[0].focus();
					}),
					(this.$onInit = function() {
						k.toolbarButtons = j.getToolbarButtons(k.buttons && string2array(k.buttons));
						var b = Boolean(k.placeholder);
						(k.ngModelController.$render = function() {
							return k.ngModelController.$viewValue
								? l.html(k.ngModelController.$viewValue)
								: b ? l.empty() : l.html('<p></p>');
						}),
							l.bind('blur keyup change focus click', function() {
								!b || (l.html().length && '<br>' !== l.html()) || l.empty(),
									k.ngModelController.$setViewValue(l.html()),
									a.$applyAsync();
							});
					}),
					l.on('paste', function(a) {
						if (d.onPaste) {
							var b = void 0;
							(b =
								window.clipboardData && window.clipboardData.getData
									? window.clipboardData.getData('Text')
									: (a.originalEvent || a).clipboardData.getData('text/plain')),
								a.preventDefault(),
								c.when(k.onPaste({ $event: a, pasteContent: b })).then(function(a) {
									pasteHtmlAtCaret(a);
								});
						}
					});
	...
	angular.module('ngWig').provider('ngWigToolbar', function() {
		...
		(this.setButtons = function(a) {
			...
			(this.addStandardButton = function(c, d, e, f) {
				...
				(f = f || ''), (a[c] = { title: d, command: e, styleClass: f }), b.push(c);
			}),
			(this.addCustomButton = function(c, d) {
				if (!c || !d) throw 'Arguments "name" and "pluginName" are required';
				(a[c] = { pluginName: d, isComplex: !0 }), b.push(c);
			}),
			(this.$get = function() {
				return {
					getToolbarButtons: function(d) {
						var e = [];
						return (
	...
	angular.module('ngWig').component('ngWigPlugin', {
		bindings: { plugin: '<', execCommand: '=', editMode: '=', disabled: '=', options: '<', content: '=' },
		controller: [
			'$scope',
			'$element',
			'$compile',
				...
				this.$onInit = function() {
					b.replaceWith(
						c(
							'<' +
								d.plugin.pluginName +
								' plugin="$ctrl.plugin"exec-command="$ctrl.execCommand"edit-mode="$ctrl.editMode"disabled="$ctrl.disabled"options="$ctrl.options"content="$ctrl.content"/>'
						)(a)
					);
				};
			}
